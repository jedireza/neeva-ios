...
class TestSQLiteReadingList: XCTestCase {
    ...
    override func setUp() {
        super.setUp()
        self.db = BrowserDB(
            filename: "ReadingListTest.db", schema: ReadingListSchema(), files: self.files)
    ...
    func testCreateRecord() {
        ...
        case .success(let result):
            XCTAssertEqual(
                result.url, "http://www.anandtech.com/show/9117/analyzing-intel-core-m-performance")
            XCTAssertEqual(
                result.title,
    ...
    func testGetRecordWithURL() {
        ...
        let result2 = readingList.getRecordWithURL(
            "http://www.anandtech.com/show/9117/analyzing-intel-core-m-performance"
        ).value
        switch result2 {
        case .failure(let error):
    ...
    func testGetAllRecords() {
        let _ = createRecordWithURL(
            "http://localhost/article1", title: "Test 1", addedBy: "Stefan's iPhone")
        let createResult2 = createRecordWithURL(
            "http://localhost/article2", title: "Test 2", addedBy: "Stefan's iPhone")
        let _ = createRecordWithURL(
            "http://localhost/article3", title: "Test 3", addedBy: "Stefan's iPhone")
    ...
    func testDeleteRecord() {
        ...
        let result3 = readingList.getRecordWithURL(
            "http://www.anandtech.com/show/9117/analyzing-intel-core-m-performance"
        ).value
        switch result3 {
        case .failure:
    ...
    func testUpdateRecord() {
        ...
        if let record = result.successValue {
            XCTAssertEqual(
                record.url, "http://www.anandtech.com/show/9117/analyzing-intel-core-m-performance")
            XCTAssertEqual(
                record.title,
                "Analyzing Intel Core M Performance: How 5Y10 can beat 5Y71 & the OEMs' Dilemma")
    ...
    func createRecordWithURL(_ url: String, title: String, addedBy: String) -> Maybe<
        ReadingListItem
    > {
