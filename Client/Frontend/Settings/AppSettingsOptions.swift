...
class NeevaSearchSetting: Setting {
    ...
    override func onConfigureCell(_ cell: UITableViewCell) {
        super.onConfigureCell(cell)
        if TourManager.shared.isCurrentStep(with: .promptSettingsInNeevaMenu) {
            let timeInterval: CFTimeInterval = 1
            let widthAnimation = CABasicAnimation(keyPath: "borderWidth")
            widthAnimation.fromValue = 2
            widthAnimation.toValue = 6
            widthAnimation.duration = timeInterval
            widthAnimation.repeatCount = Float.infinity
            cell.layer.borderWidth = 4
            cell.layer.cornerRadius = 8
            cell.layer.borderColor = UIColor.Neeva.Tour.Background.cgColor
            cell.layer.add(widthAnimation, forKey: "borderWidthAnimation")
        }
    }
    ...
    override func onClick(_ navigationController: UINavigationController?) {
        ...
        if TourManager.shared.hasActiveStep() {
            TourManager.shared.setLastTap(.accountSetting)
        }
        navigationController?.dismiss(animated: true) {
            if TourManager.shared.isCurrentStep(with: .promptSettingsInNeevaMenu) {
                TourManager.shared.responseMessage(for: .promptSettingsInNeevaMenu)
