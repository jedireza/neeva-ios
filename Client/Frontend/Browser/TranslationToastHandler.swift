...
class TranslationToastHandler: TabEventHandler {
    ...
    func tab(_ tab: Tab, didDeriveMetadata metadata: DerivedMetadata) {
        ...
        guard let myLanguage = Locale.autoupdatingCurrent.languageCode,
            let pageLanguage = metadata.language
        else {
            return
        }

        guard let url = tab.url,
            !url.absoluteString.starts(with: setting.useTranslationService.destinationURLPrefix)
        else {
            return
        }
        let pageLocale = Locale(identifier: pageLanguage)

        if pageLocale.languageCode != Locale(identifier: myLanguage).languageCode,
            pageLocale.languageCode != "mul"
        {
    ...
    func promptTranslation(_ tab: Tab, from pageLanguage: String, to myLanguage: String) {
        ...
        let localizedPageLanguage =
            locale.localizedString(forLanguageCode: pageLanguage) ?? pageLanguage

        let service = setting.useTranslationService
        let promptMessage = String(
            format: Strings.TranslateSnackBarPrompt, localizedPageLanguage, localizedMyLanguage,
            service.name)
        let snackBar = SnackBar(
            text: promptMessage, img: UIImage(named: "search"),
            snackbarClassIdentifier: snackBarClassIdentifier)
        let cancel = SnackButton(
            title: Strings.TranslateSnackBarNo,
            accessibilityIdentifier: "TranslationPrompt.dontTranslate", bold: false
        ) { bar in
            tab.removeSnackbar(bar)

            TelemetryWrapper.recordEvent(
                category: .action, method: .translate, object: .tab,
                extras: ["action": "decline", "from": pageLanguage, "to": myLanguage])
        }
        let ok = SnackButton(
            title: Strings.TranslateSnackBarYes,
            accessibilityIdentifier: "TranslationPrompt.doTranslate", bold: true
        ) { bar in
            tab.removeSnackbar(bar)
        ...
        TelemetryWrapper.recordEvent(
            category: .prompt, method: .translate, object: .tab,
