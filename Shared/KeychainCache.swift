...
open class KeychainCache<T: JSONLiteralConvertible> {
    ...
    open class func fromBranch(
        _ branch: String, withLabel label: String?, withDefault defaultValue: T? = nil,
        factory: (JSON) -> T?
    ) -> KeychainCache<T> {
        if let l = label {
            let key = "\(branch).\(l)"
            KeychainWrapper.sharedAppContainerKeychain.ensureStringItemAccessibility(
                .afterFirstUnlock, forKey: key)
    ...
    open func checkpoint() {
            ...
            let jsonString = value.asJSON().stringify()
        {
            KeychainWrapper.sharedAppContainerKeychain.set(
                jsonString, forKey: "\(branch).\(label)", withAccessibility: .afterFirstUnlock)
        } else {
