...
open class AsyncReducer<T, U> {
    ...
    public convenience init(
        initialValue: T, queue: DispatchQueue = DefaultDispatchQueue, combine: @escaping Combine
    ) {
        self.init(initialValue: deferMaybe(initialValue), queue: queue, combine: combine)
    }

    public init(
        initialValue: Deferred<Maybe<T>>, queue: DispatchQueue = DefaultDispatchQueue,
