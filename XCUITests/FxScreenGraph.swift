...
class Action {
    ...
    static let SelectNewTabAsNeevaHomePage = "SelectNewTabAsNeevaHomePage"
    static let SelectNewTabAsCustomURL = "SelectNewTabAsCustomURL"

    static let SelectHomeAsNeevaHomePage = "SelectHomeAsNeevaHomePage"
    ...
    static let RequestDesktopSiteViaReloadMenu = "RequestDesktopSiteViaReloadMenu"
    static let RequestMobileSiteViaReloadMenu = "RequestMobileSiteViaReloadMenu"
}
...
func createScreenGraph(for test: XCTestCase, with app: XCUIApplication) -> MMScreenGraph<FxUserState> {
    ...
    map.addScreenState(NewTabSettings) { screenState in
        ...
        screenState.gesture(forAction: Action.SelectNewTabAsNeevaHomePage) { UserState in
    ...
    map.addScreenState(HomeSettings) { screenState in
        screenState.gesture(forAction: Action.SelectHomeAsNeevaHomePage) { UserState in
    ...
    map.addScreenState(TabTray) { screenState in
        ...
        screenState.tap(app.buttons["TabTrayController.maskButton"], forAction: Action.ToggleRegularMode) { userState in
            userState.isPrivate = !userState.isPrivate
        }
    ...
    map.addScreenState(ReloadLongPressMenu) { screenState in
        ...
        screenState.tap(app.buttons["Request Desktop Site"], forAction: Action.RequestDesktopSiteViaReloadMenu) { userState in
            userState.requestDesktopSite = true
        }
        screenState.tap(app.buttons["Request Mobile Site"], forAction: Action.RequestMobileSiteViaReloadMenu) { userState in
            userState.requestDesktopSite = false
        }
    ...
    map.addScreenState(ShareMenu) {screenState in
        screenState.tap(app.collectionViews.buttons["Copy"], forAction: Action.CopyAddressPAM)
        screenState.tap(app.collectionViews.buttons["Find on Page"], to: FindInPage)
        screenState.tap(app.collectionViews.buttons["Request Desktop Site"], to: RequestDesktopSite)
        screenState.tap(app.collectionViews.buttons["Request Mobile Site"], to: RequestMobileSite)
        screenState.backAction = cancelBackAction
        screenState.dismissOnUse = true
    }
    ...
    map.addScreenState(NeevaMenu) { screenState in
        screenState.tap(app.buttons["NeevaMenu.Settings"], to: SettingsScreen)
        screenState.tap(app.buttons["NeevaMenu.History"], to: LibraryPanel_History)
        screenState.tap(app.buttons["NeevaMenu.Downloads"], to: LibraryPanel_Downloads)
        screenState.tap(app.buttons["NeevaMenu.Home"], forAction: Action.GoToHomePage) { userState in }
...
extension MMNavigator where T == FxUserState {
    ...
    func browserPerformAction(_ view: BrowserPerformAction) {
        ...
        let BrowserMenuOptions = [.openTopSitesOption, BrowserPerformAction.openSettingsOption]
