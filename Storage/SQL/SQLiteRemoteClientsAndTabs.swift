...
open class SQLiteRemoteClientsAndTabs: RemoteClientsAndTabs {
    ...
    class func remoteClientFactory(_ row: SDRow) -> RemoteClient {
        ...
        return RemoteClient(
            guid: guid, name: name, modified: mod, type: type, formfactor: form, os: os,
            version: version, fxaDeviceId: fxaDeviceId)
    }
    ...
    class func remoteTabFactory(_ row: SDRow) -> RemoteTab {
        ...
        return RemoteTab(
            clientGUID: clientGUID, URL: url, title: title, history: history, lastUsed: lastUsed,
            icon: nil)
    }
    ...
    class func convertHistoryToString(_ history: [URL]) -> String? {
        ...
        guard let data = try? JSONSerialization.data(withJSONObject: historyAsStrings, options: [])
        else {
            return nil
        }
        return String(
        ...
        return db.transaction { connection -> Int in
            ...
            for tab in tabs {
                ...
                try connection.executeChange(
                    "INSERT INTO tabs (client_guid, url, title, history, last_used) VALUES (?, ?, ?, ?, ?)",
                    withArgs: args)
    ...
    open func insertOrUpdateClients(_ clients: [RemoteClient]) -> Deferred<Maybe<Int>> {
        ...
        return db.transaction { connection -> Int in
            ...
            for client in clients {
                ...
                try connection.executeChange(
                    "UPDATE clients SET name = ?, modified = ?, type = ?, formfactor = ?, os = ?, version = ?, fxaDeviceId = ? WHERE guid = ?",
                    withArgs: args)
                ...
                if connection.numberOfRowsModified == 0 {
                    ...
                    try connection.executeChange(
                        "INSERT INTO clients (guid, name, modified, type, formfactor, os, version, fxaDeviceId) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
                        withArgs: args)
    ...
    open func getClient(guid: GUID) -> Deferred<Maybe<RemoteClient?>> {
        ...
        return self.db.runQuery(
            "SELECT * FROM clients WHERE guid = ?", args: [guid], factory: factory) >>== {
                deferMaybe($0[0])
            }
    }
    ...
    open func getClient(fxaDeviceId: String) -> Deferred<Maybe<RemoteClient?>> {
        let factory = SQLiteRemoteClientsAndTabs.remoteClientFactory
        return self.db.runQuery(
            "SELECT * FROM clients WHERE fxaDeviceId = ?", args: [fxaDeviceId], factory: factory)
            >>== { deferMaybe($0[0]) }
    }
    ...
    open func getClientGUIDs() -> Deferred<Maybe<Set<GUID>>> {
        let c = db.runQuery(
            "SELECT guid FROM clients WHERE guid IS NOT NULL", args: nil,
            factory: { $0["guid"] as! String })
        return c >>== { cursor in
    ...
    open func getTabsForClientWithGUID(_ guid: GUID?) -> Deferred<Maybe<[RemoteTab]>> {
        ...
        return db.runQuery(
            tabsSQL, args: clientArgs, factory: SQLiteRemoteClientsAndTabs.remoteTabFactory) >>== {
                let tabs = $0.asArray()
    ...
    open func getClientsAndTabs() -> Deferred<Maybe<[ClientAndTabs]>> {
        return db.withConnection { conn -> ([RemoteClient], [RemoteTab]) in
            let clientsCursor = conn.executeQuery(
                "SELECT * FROM clients WHERE EXISTS (SELECT 1 FROM remote_devices rd WHERE rd.guid = fxaDeviceId) ORDER BY modified DESC",
                factory: SQLiteRemoteClientsAndTabs.remoteClientFactory)
            let tabsCursor = conn.executeQuery(
                "SELECT * FROM tabs WHERE client_guid IS NOT NULL ORDER BY client_guid DESC, last_used DESC",
                factory: SQLiteRemoteClientsAndTabs.remoteTabFactory)
        ...
        -> Deferred<Maybe<Int>>
    {
        return db.transaction { connection -> Int in
            ...
            for command in commands {
                for client in clients {
                    do {
                        if let commandID = try self.insert(
                            connection,
                        ...
                        log.error(
                            "insertCommands(_:, forClients:) failed: \(err.localizedDescription) (numberOfInserts: \(numberOfInserts)"
                        )
                        throw err
                    }
                }
...
extension SQLiteRemoteClientsAndTabs: RemoteDevices {
    open func replaceRemoteDevices(_ remoteDevices: [RemoteDevice]) -> Success {
        ...
        let remoteDevices = remoteDevices.filter {
            $0.id != nil && $0.type != nil && !$0.isCurrentDevice
        }
        ...
        return db.transaction { conn -> Void in
            ...
            for device in remoteDevices {
                ...
                let args: Args = [
                    device.id, device.name, device.type, device.isCurrentDevice, now, now,
                    device.lastAccessTime, availableCommands,
                ]
