...
open class SQLiteReadingList {
    ...
    let allColumns = [
        "client_id", "client_last_modified", "id", "last_modified", "url", "title", "added_by",
        "archived", "favorite", "unread",
    ].joined(separator: ",")
...
extension SQLiteReadingList: ReadingList {
    ...
    public func createRecordWithURL(_ url: String, title: String, addedBy: String) -> Deferred<
        Maybe<ReadingListItem>
    > {
        return db.transaction { connection -> ReadingListItem in
            let insertSQL =
                "INSERT OR REPLACE INTO items (client_last_modified, url, title, added_by) VALUES (?, ?, ?, ?)"
            ...
            let cursor = connection.executeQuery(
                querySQL, factory: SQLiteReadingList.ReadingListItemFactory, withArgs: queryArgs)
    ...
    public func getRecordWithURL(_ url: String) -> Deferred<Maybe<ReadingListItem>> {
        ...
        let args: Args = [url]
        return db.runQuery(sql, args: args, factory: SQLiteReadingList.ReadingListItemFactory)
            >>== { cursor in
                let items = cursor.asArray()
                if let item = items.first {
    ...
    public func updateRecord(_ record: ReadingListItem, unread: Bool) -> Deferred<
        Maybe<ReadingListItem>
    > {
        return db.transaction { connection -> ReadingListItem in
    ...
    fileprivate class func ReadingListItemFactory(_ row: SDRow) -> ReadingListItem {
        ...
        return ReadingListItem(
            id: id, lastModified: lastModified, url: url, title: title, addedBy: addedBy,
            unread: unread, archived: archived, favorite: favorite)
    }
}
