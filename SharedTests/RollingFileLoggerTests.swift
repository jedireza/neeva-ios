...
class RollingFileLoggerTests: XCTestCase {
    ...
    override func setUp() {
        ...
        do {
            try FileManager.default.createDirectory(
                atPath: logDir, withIntermediateDirectories: false, attributes: nil)
        } catch _ {
        }
        logger = RollingFileLogger(
            filenameRoot: "test", logDirectoryPath: logDir, sizeLimit: Int64(sizeLimit))
    }
    ...
    func testNewLogCreatesLogFileWithTimestamp() {
        ...
        XCTAssertTrue(
            logString!.contains(testMessage), "Log should contain our test message that we wrote")
    }
    ...
    func testNewLogDeletesPreviousLogIfItsTooLarge() {
        ...
        let directorySize = try! manager.getAllocatedSizeOfDirectoryAtURL(
            dirURL, forFilesPrefixedWith: prefix)
        ...
        let exceedsSmaller = try! manager.allocatedSizeOfDirectoryAtURL(
            dirURL, forFilesPrefixedWith: prefix, isLargerThanBytes: directorySize - 1)
        let doesNotExceedLarger = try! manager.allocatedSizeOfDirectoryAtURL(
            dirURL, forFilesPrefixedWith: prefix, isLargerThanBytes: Int64(sizeLimit + 2))
